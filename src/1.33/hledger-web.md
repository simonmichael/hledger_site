<!-- Generated by "Shake webmanuals" from hledger-web/hledger-web.m4.md doc/common.m4 hledger/Hledger/Cli/Commands/commands.m4 hledger-web/.version.m4 hledger-web/.date.m4 -->
...
:   set the user\'s access level for changing data (default: `add`). It
    also accepts `sandstorm` for use on that platform (reads permissions
    from the `X-Sandstorm-Permissions` request header).
...
Note that hledger-web shows accounts with zero balances by default, like
`hledger-ui` (and unlike `hledger`). Using the `-E/--empty` flag at
...
when costs are hidden. Currently hledger-web does not show costs at all.
