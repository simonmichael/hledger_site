...
see fewer decimal places in reports, use a [commodity
directive](#declaring-commodities) to override that.

hledger uses [banker's
...
"0"). (Note, prior to hledger 1.17.1 this could vary if hledger happened
to be built with an old version of Decimal (\<0.5.1); since 1.17.1 it's
guaranteed.)
...
displayed with zero decimal digits is "0". (More at [Amount display
style](#amount-display-style).)
