...
This manual is for hledger\'s command line interface, version 1.51.2. It
...
2.  Field matchers expect either a CSV field number, or a [CSV field
    name](#field-names) declared with [`fields`](#fields-list). (Don\'t
    use a hledger field name here, unless it is also a CSV field name.)
    A non-CSV field name will cause the matcher to match against `""`
    (the empty string), and does not raise an error, allowing easier
    reuse of common rules with different CSV files.
...
use `check accounts` or `check --strict`.
...
1.  In a powershell window, run
    `[Environment]::SetEnvironmentVariable("LEDGER_FILE", "C:\User\USERNAME\finance\my.journal", [System.EnvironmentVariableTarget]::User)`
