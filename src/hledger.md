...
<!-- Generated by "Shake webmanuals" from hledger/hledger.m4.md doc/common.m4 hledger/.version.m4 -->
...
:   'always' or 'yes': always, useful eg when piping output into 'less
    -R'.
:   'never' or 'no': never.
:   A NO\_COLOR environment variable overrides this.
...
Additionally, if the command is an [add-on](#commands), you may need to
...
arguments once, so you might need to *triple*-escape. Eg in bash,
...
-   use a single parent file which
    [includes](journal.html#including-other-files) the others
...
5.  they do not support
    ...
    aliases](journal.html#regex-aliases), where
    [backreferences](https://www.regular-expressions.info/backref.html)
...
1.  A *declared market price* or *inferred market price*: A's latest
    ...
    [P directive](journal.html#declaring-market-prices), or (with the
    `--infer-value` flag) inferred from [transaction
...
3.  A *a forward chain of market prices*: a synthetic price formed by
    combining the shortest chain of "forward" (only 1 above) market
    prices, leading from A to B.

4.  A *any chain of market prices*: a chain of any market prices,
...
-   [import](#import) - add any new transactions from other files (eg
    csv)
...
-   [balance](#balance) (b, bal) - show balance changes/end
    balances/budgets in accounts
...
-   [print-unique](#print-unique) - show only transactions with unique
    descriptions
-   [register](#register) (r, reg) - show postings in one or more
...
Accounts are sorted by [declaration
order](https://hledger.org/hledger.html#declaring-accounts) if any, and
then by account name. Accounts which have zero balance are not shown
unless `-E/--empty` is used. The reported balances' total is shown as
the last line, unless disabled by `-N`/`--no-total`.
...
account itself, if there was any). Note this causes some repetition, and
the final total (`0`) is the sum of the top-level balances, not of all
the balances shown.

Each group of sibling accounts is sorted separately, by [declaration
order](https://hledger.org/hledger.html#declaring-accounts) and then by
account name.

#### Multi-period balance report

Multi-period balance reports are a very useful hledger feature,
activated if you provide one of the [reporting
interval](#reporting-interval) flags, such as `-M`/`--monthly`. They are
similar to single-period balance reports, but they show the report as a
table, with columns representing one or more successive time periods.
...
budget goals are defined by [periodic
...
-   **autobalanced** - all transactions are
    [balanced](journal.html#postings), inferring missing amounts where
    necessary, and possibly converting commodities using [transaction
    prices](journal.html#transaction-prices) or automatically-inferred
    transaction prices

-   **assertions** - all [balance
    assertions](journal.html#balance-assertions) in the journal are
...
used ([strict mode](hledger.html#strict-mode)):

-   **accounts** - all account names used by transactions [have been
    declared](journal.html#account-error-checking)

-   **commodities** - all commodity symbols used [have been
    declared](journal.html#commodity-error-checking)
...
as [add-on commands](hledger.html#add-on-commands) in
...
styles](journal.html#commodity-display-style) (declared or inferred) in
...
beginning of they year, and then after that our \$100 investment gets
\$1 increase in value, or 1% of \$100. Let's take a closer look at what
is happening here by asking for quarterly reports instead of annual:
...
-   [ui](hledger-ui.html) an efficient terminal interface for hledger
    (TUI)
...
-   [iadd](http://hackage.haskell.org/package/hledger-iadd) a more
    interactive alternative for the [add](hledger.html#add) command
...
position deciding which program they refer to. Eg `hledger -h web` shows
...
**NO\_COLOR** If this variable exists with any value, hledger will not
