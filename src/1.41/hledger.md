...
*(Since 1.41:)* You can use a negated `!` matcher on a `&` line, meaning
AND NOT.
...
The expression should be enclosed in quotes. See [Combining query
terms](#combining-query-terms) below.
...
`-s`/`--strict` flag is used ([strict mode](#strict-mode)). Strict mode
always enables the balance assertions check, also. These provide extra
