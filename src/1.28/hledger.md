...
check by the [bse](#balancesheetequity) report\'s zero total.
...
with `--close-acct` and `--open-acct`. (If you specify only one of
...
With `--interleaved`, each equity posting is shown next to the posting
...
specifying a [report end date](#report-start--end-date), where \"last
...
year-end balances. If you find yourself needing more precise
[queries](#queries), here\'s one solution: add more easily-matched tags
to opening/closing transactions, like this:
...
`status:`) with `close`, or the generated balance assertions will depend
on these flags. Likewise, if you run this command with `--auto`, the
...
are using @/@@ notation without [equity
postings](/investments.html#a-more-correct-entry)):
...
prices\
Print [market price
directives](https://hledger.org/manual#market-prices) from the journal.
With \--infer-market-prices, generate additional market prices from
[transaction prices](https://hledger.org/manual#transaction-prices).
...
prices](https://hledger.org/hledger.html#transaction-prices) are
...
hledger repo\'s [bin/
directory](https://github.com/hledgerorg/hledger/tree/master/bin).
...
(`accounttag`) - the transaction has two tags (`transaction-tag`,
`another-transaction-tag`) - the `assets:checking` posting has three
tags (`transaction-tag`, `another-transaction-tag`, `accounttag`) - the
`expenses:food` posting has three tags (`transaction-tag`,
`another-transaction-tag`, `posting-tag`).
...
`a-posting-tag`\'s value is \"the tag value\", `tag2`\'s value is
\"foo\", and `tag3`\'s value is \"\" (the empty string):
...
[`decimal-mark`](#declaring-the-decimal-mark) directive is best,
otherwise [`commodity`](#declaring-commodities) directives will also
...
amounts](#transaction-prices), and all amounts displayed by the
[`print`](#print) command, are displayed with all of their decimal
digits visible.)
...
another commodity, by writing either `@ UNITPRICE` or `@@ TOTALPRICE`
...
because it is specific to one transaction, unlike [market
prices](#declaring-market-prices) which are not. \"Cost\" is shorter and
might be preferable; just remember this feature can represent either a
buyer\'s cost, or a seller\'s price.
...
4.  Like 1, but the `@` is parenthesised, i.e. `(@)`; this is for
    compatibility with Ledger journals ([Virtual posting
...
price](http://ledger-cli.org/3.0/doc/ledger3.html#Fixing-Lot-Prices)
(four variants: `{UNITPRICE}`, `{{TOTALPRICE}}`, `{=FIXEDUNITPRICE}`,
...
currently ignores them. A [transaction price](#transaction-prices), lot
...
directive](#including-other-files) are processed as if concatenated into
...
directive](#declaring-commodities) may limit the display precision, but
...
probably add some as your needs grow. Here is an overview of directives
by purpose:

| purpose                                                                         | directives                                                                                                                                                | command line options with similar effect    |
|---------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------|
| **READING/GENERATING DATA:**                                                    |                                                                                                                                                           |                                             |
| Declare a commodity\'s or file\'s decimal mark to help parse amounts accurately | [`commodity`](#declaring-commodities), [`D`](#default-commodity), [`decimal-mark`](#decimal-mark)                                                         |                                             |
| Apply changes to the data while parsing                                         | [`alias`](#rewriting-accounts), [`apply account`](#default-parent-account), [`comment`](#comment-blocks), [`D`](#default-commodity), [`Y`](#default-year) | [`--alias`](#rewriting-accounts)            |
| Inline extra data files                                                         | [`include`](#including-other-files)                                                                                                                       | [multiple `-f/--file`\'s](#multiple-files)  |
| Generate extra transactions or budget goals                                     | [`~`](#periodic-transactions)                                                                                                                             |                                             |
| Generate extra postings                                                         | [`=`](#auto-postings)                                                                                                                                     |                                             |
| **CHECKING FOR ERRORS:**                                                        |                                                                                                                                                           |                                             |
| Define valid entities to allow stricter error checking                          | [`account`](#declaring-accounts), [`commodity`](#declaring-commodities), [`payee`](#declaring-payees)                                                     |                                             |
| **DISPLAYING REPORTS:**                                                         |                                                                                                                                                           |                                             |
| Declare accounts\' display order and accounting type                            | [`account`](#declaring-accounts)                                                                                                                          |                                             |
| Declare commodity display styles                                                | [`commodity`](#declaring-commodities), [`D`](#default-commodity)                                                                                          | [`-c/--commodity-style`](#commodity-styles) |
...
| **[`commodity`](#declaring-commodities)**      | Declares a commodity, for checking all entries in all files; <br>the decimal mark for parsing amounts of this commodity, for following entries until end of current file; <br>and its display style, for reports. Takes precedence over `D`. <br>Subdirectives: [`format`](#format) (alternate syntax). | N, <br>Y<br><br>  |
| **[`D`](#default-commodity)**                  | Sets a default commodity to use for no-symbol amounts, <br>and its decimal mark for parsing amounts of this commodity in following entries until end of current file; <br>and its display style, for reports.                                                                                           | Y                 |
...
| **[`include`](#including-other-files)**        | Includes entries and directives from another file, as if they were written inline.                                                                                                                                                                                                                      |                   |
| **[`payee`](#declaring-payees)**               | Declares a payee name, for checking all entries in all files.                                                                                                                                                                                                                                           |                   |
| **[`P`](#market-prices)**                      | Declares a market price for a commodity on some date, for [valuation](#valuation) reports.                                                                                                                                                                                                              |                   |
| **[`Y`](#default-year)**                       | Declares a year for yearless dates, for following entries until end of current file.                                                                                                                                                                                                                    | Y                 |
| **[`~`](#periodic-transactions)** (tilde)      | Declares a periodic transaction rule that generates future transactions with `--forecast` and budget goals with `balance --budget`.                                                                                                                                                                     |                   |
...
been declared by a [`commodity` directive](#declaring-commodities). This
...
directive](#declaring-commodities) (setting the commodity\'s decimal
mark for parsing and [display style](#amount-display-format) for
output).
...
If you are using `D` and also [checking](#check) commodities, you will
need to add a `commodity` directive similar to the `D`. (The
`hledger check commodities` command expects `commodity` directives, and
ignores `D`).
...
-   They can store additional account information as
    [comments](#comments), or as [tags](#tags) which can be used to
    filter or pivot reports.
...
-   can be written on the same line, but only after **two or more
    spaces** (because `;` is allowed in account names)
...
directive](#declaring-accounts). Some notes:
...
-   Accounts can only be declared in `journal` files, but will affect
    [included](#including-other-files) files of all types.
...
explicitly, by adding a `type:` [tag](#tags-1) to your top-level account
...
-   The rules for inferring types from account names are as follows.
    ...
    expressions](#regular-expressions). Note the Cash regexp changed in
    hledger 1.24.99.2.
...
-   Certain uses of [account aliases](#account-aliases) can disrupt
...
[included files](#including-other-files) (but note: [not sibling or
parent files](#aliases-and-multiple-files)). The spaces around the = are
optional:
...
define budget goals, shown in [budget reports](#budget-report).
...
containing dated records as if they were journal files, automatically
converting each CSV record into a transaction.
...
It also has a second purpose: it can be used inside [if
blocks](#if-block) to ignore certain CSV records (described below).
...
(The other way is [field assignments](#field-assignment), see below.) A
fields list does does two things:
...
record (`%N`), or by the name they were given in the [fields
list](#fields-list) (`%CSVFIELDNAME`).
...
can use in a [fields list](#fields-list) and in [field
assignments](#field-assignment). For more about the transaction parts
they refer to, see [Transactions](#transactions).
...
REGEX is a case-insensitive [regular expression](#regular-expressions)
...
(`\b`, `\B`, `\<`, `\>`), and nothing else. If you have trouble, be sure
...
table and, like with `if` blocks, later rules (in the same or another
table) or `if` blocks could override the effect of any rule.

Instead of \',\' you can use a variety of other non-alphanumeric
characters as a separator. First character after `if` is taken to be the
...
This rule can be used inside [if blocks](#if-block) (only), to make
hledger stop reading this CSV file and move on to the next input file,
or to command execution. Eg:
...
You can override the file extension with a [separator](#separator) rule
if needed. See also: [Input files](#input-files) in the hledger manual.
...
1.  **If the CSV has a single amount field:**\
    Assign (via a [fields list](#fields) or a [field
    assignment](#field-assignment)) to `amountN`. This sets the Nth
    posting\'s amount. N is usually 1 or 2 but can go up to 99.
...
2.  **If the CSV has separate amount fields for debit & credit (in &
    ...
    a.  **If both fields are unsigned:**\
        ...
        amount to whichever of these has a non-zero value, and negates
        the \"-out\" value.

    b.  **If either field is signed (can contain a minus sign):**\
        Use a [conditional rule](#if-block) to flip the sign (of
        non-empty values). Since hledger always negates amountN-out, if
        it was already negative, we must undo that by negating once more
    ...
    c.  **If both fields, or neither field, can contain a non-zero
        ...
        non-zero value. Eg, the `amountN-in`/`amountN-out` rules would
        reject value pairs like these:
    ...
    So, use smarter [conditional rules](#if-block) to set the amount
    from the appropriate field. Eg, these rules would make it use only
    the value containing non-zero digits, handling the above:
...
4.  **If the CSV has the balance instead of the transaction amount:**\
    ...
    a [balance assignment](#balance-assignments). (Old syntax:
    `balance`, equivalent to `balance1`.)

    -   **If hledger guesses the wrong default account name:**\
        When setting the amount via balance assertion, hledger may guess
        the wrong default account name. So, set the account name
        explicitly, eg:
...
-   One or more stars (`*`) followed by a space, at the start of a line,
    is ignored. So date lines or time transaction lines can also be
    Org-mode headlines.
