...
This manual is for hledger\'s command line interface, version 1.31. It
...
When amounts are converted to other commodities in
[cost](#cost-reporting) or [value](#value-reporting) reports, note that
`cur:` matches the new commodity symbol, and not the old one, and `amt:`
...
-   Note: bare layout will sometimes display an extra row for the
    ...
    confusingly (workaround: add a `cur:` query to exclude the no-symbol
    row).
