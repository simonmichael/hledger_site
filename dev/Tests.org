...
As a user, you can also run the unit tests, which are built in to the hledger executable as the [[http://hledger.org/manual#test][test command]].
Tests run this way are expected to pass - if not, something has gone wrong, please let us know!
